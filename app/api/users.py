...
def create_user():
    ...
    # The code that is used when a new entity has been created: 201
...
def delete_user(id):
    ...
    # Delete user posts
    for m in Message.query.filter_by(sender_id=id).all():
        db.session.delete(m)
    # Delete user posts
    for p in Post.query.filter_by(user_id=id).all():
        db.session.delete(p)
    # Delete user
    db.session.delete(user)
    db.session.commit()
    response = jsonify({'Success': 'User {} deleted'.format(user.username)})
    return response
